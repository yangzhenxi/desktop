          ...
          <a-col
            ...
            <a-form-item
              ...
              <a-input-number
                :min="1"
                :max="10"
                v-decorator="['Number', { rules: [{required: true, message: '请输入起始数字'}] }]" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item
              ...
              <a-input-number
                :min="1"
                :max="10"
                v-decorator="['count', { rules: [{required: true, message: '请输入桌面数量'}] }]" />
            </a-form-item>
          </a-col>
        </a-row>
...
export default {
    ...
    methods: {
        ...mapMutations(['SET_FORMVALUE', 'SET_VERSION', 'SET_TPLIST', 'SET_OULIST']),
        close () {
            this.$emit('close')
        },
        nextStep () {
        ...
        async getOUList () {
            ...
            this.SET_OULIST(this.ArrOU)
            this.TPList = deepGet(await DesktopTemplateAllList(), 'data', [])
            if (this.TPList.length > 0) {
              this.TPList.forEach(u => { u.id = parseInt(u.id) })
            }
            this.SpinningLoading = false
        },
        async getVersion (e) {
            this.version = deepGet(
                await DesktopTemplateVersionList({ templateId: e }),
                'data',
                []
            )
