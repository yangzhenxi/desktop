<template>
  <a-modal
    title="新增用户组"
    :width="640"
    :visible="visible"
    :confirmLoading="confirmLoading"
    @ok="handleSubmit"
    @cancel="handleCancel"
    destroyOnClose>
    <a-spin :spinning="loading">
      <a-form :form="form">
        <a-form-item
          label="名称"
          :labelCol="labelCol"
          :wrapperCol="wrapperCol">
          <a-input
            placeholder="请输入名称"
            v-decorator="['name',
                          { rules: [
                              { required: true, message: '请输入名称！' },
                              { min: 2,max:12, message: '用户组的长度为2-12' },
                              {validator}
                            ],
                            validateFirst: true
                          }]" />
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
</template>

<script>
import { usermanageAllUser } from '@/api/CloudDesktop/userManage'
import { mixinFormModal } from '@/utils/mixin'
import { usermanageAddusergroup } from '@/api/CloudDesktop/usergroup'
import { UserGroupTree } from '@/utils/validator'
import { debounce } from '@/utils/util'
export default {
  mixins: [mixinFormModal],
  name: 'UsergroupTabAdd',
  data () {
    return {
        baseDN: null,
		validatorName: [],
		userAll: []
    }
  },
  methods: {
    async Add (TPgroupNameList) {
		this.visible = true
		this.validatorName = TPgroupNameList
		this.userAll = []
		this.userAll = this.deepGet(await usermanageAllUser({ name: 'ou=Users,ou=Citrix,dc=cloud,dc=com' }), 'list', [])
    },
    handleSubmit () {
      this.form.validateFields(async (errors, values) => {
        if (!errors) {
            this.confirmLoading = true
            values.baseDN = 'ou=Group,ou=Citrix,dc=cloud,dc=com'
            usermanageAddusergroup(values).then(res => {
                this.$message.success('添加成功')
                this.$emit('ok', res)
                this.visible = false
            })
            this.confirmLoading = false
        }
      })
    },
            // 校验重名称
    validator: debounce(function (rule, value, callback) {
        UserGroupTree({
            data: () => this.validatorName,
			userData: () => this.userAll,
            field: 'name'
        },
        { rule, value, callback }
        )
      })
  }
}
</script>

<style lang="less" scoped>
/deep/.ant-modal-title,/deep/.ant-form-item-required{
  color: white !important;
}
/deep/input#name {
        background: content-box;
        height: 0;
        padding: 1.2em .5em;
        color: white !important;
}
/deep/input#name::first-line{
      color: white;
}
/deep/i.anticon.anticon-close.ant-modal-close-icon {
    color: white;
}
</style>
