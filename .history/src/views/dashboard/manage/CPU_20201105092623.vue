        ...
        handleClick (value, title) {
            ...
            if (title === 'CPU') {
                Arr = this.cpu.filter(u => u.host === value)
                 this.dataSource[0].ChartsData[1].count = (Arr.cpu_usage / 1000).toFixed(2)
            this.dataSource[0].ChartsData[0].count = (Arr.cpuCapacity - (Arr.cpu_usage / 1000)).toFixed(2)
            this.dataSource[1].ChartsData[1].count = (memoryUsage / 24 / 1024 / 1024) - (memoryUsage / 1024)).toFixed(2)
            } else {
