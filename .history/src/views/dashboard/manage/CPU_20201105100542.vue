        ...
        <m-card class="card" hover :style="i.style">
          <div class="header">
            ...
            <a-dropdown :trigger="['click']" placement="bottomCenter">
              ...
              <a-menu slot="overlay">
                <a-menu-item v-for="(item,index) in i.Cluster" :key="index" @click="handleClick(item,i.title)">
                  {{ item }}
                </a-menu-item>
              </a-menu>
            </a-dropdown>
        ...
        handleClick (value, title) {
            ...
            if (title === 'CPU') {
                ...
                const Used = {
                    ...
                    percent: (Arr[0].cpu_usage / 1000) / (Arr[0].cpu_capacity - (Arr[0].cpu_usage / 1000))
                }
                const Surplus = {
                    item: 'CPU剩余容量',
                    count: (Arr[0].cpu_capacity - (Arr[0].cpu_usage / 1000)).toFixed(2),
                    percent: 1 - (Arr[0].cpu_usage / 1000) / (Arr[0].cpu_capacity - (Arr[0].cpu_usage / 1000))
                }
                this, dataSource[0].ChartsData[1] = [Surplus, Used]
                this.dataSource[0].ChartsData[1].count = (Arr[0].cpu_usage / 1000).toFixed(2)
                this.dataSource[0].ChartsData[0].count =
                this.dataSource[0].ChartsData[0].percent = 1 - (Arr[0].cpu_usage / 1000) / (Arr[0].cpu_capacity - (Arr[0].cpu_usage / 1000))
                this.dataSource[0].ChartsData[1].percent = (Arr[0].cpu_usage / 1000) / (Arr[0].cpu_capacity - (Arr[0].cpu_usage / 1000))
            } else {
                Arr = this.memory.filter(u => u.host === value)
                this.dataSource[1].ChartsData[1].count = (Arr[0].memory_usage / 1024).toFixed(2)
                this.dataSource[1].ChartsData[0].count = ((Arr[0].memory_capacity / 1024 / 1024 / 1024) - (Arr[0].memory_usage / 1024)).toFixed(2)
                this.dataSource[1].ChartsData[0].percent = 1 - ((Arr[0].memory_usage / 1024).toFixed(2) / ((Arr[0].memory_capacity / 1024 / 1024 / 1024)))
                this.dataSource[1].ChartsData[1].percent = (Arr[0].memory_usage / 1024).toFixed(2) / ((Arr[0].memory_capacity / 1024 / 1024 / 1024))
            }
        },
        deepGet
    }
