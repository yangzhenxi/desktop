...
export default {
  ...
  methods: {
    ...
    async handleSubmit () {
      ...
      this.AllData.forEach((u, index) => {
        if (index > 0 && index < this.AllData.length - 1) {
          const row = u.split(',')
          row.forEach((o, sum) => {
            //   console.log(o.toString() + '---' + sum.toString())
            console.log(this.ErrorArr[sum] + '---' + o)
            if (this.titleNameChange(this.ErrorArr[sum], o) === 1) this.$message.error(this.ErrorArr[sum] + '格式错误')
          })
        }
      })
    ...
    username (value) {
      ...
      const target = this.validatorName.find((u) => u[name] === value)
      if (target) {
        return 1
      }
      this.validatorName.push({ name: value })
    },
