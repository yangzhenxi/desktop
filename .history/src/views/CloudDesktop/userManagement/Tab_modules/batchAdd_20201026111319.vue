    ...
    methods: {
        ...
        beforeUpload (file) {
            ...
            readers.onloadend = evt => {
                evt.target.result.substring(0, 3) === '用户名' ? reader.readAsText(file, 'GB2312') : reader.readAsText(file, 'UTF-8')
            }
        ...
        option () {
            ...
            const content = new Blob(['uFEFF' + csv], { type: 'text/csv;charset=utf-8' })
            // const content = new Blob([csv], { type: 'text/csv;charset=ANSI' })
            console.log(content)
            // 生成url对象
