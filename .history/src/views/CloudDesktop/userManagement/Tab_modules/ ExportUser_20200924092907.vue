  ...
  methods: {
        ...
        if (!errors) {
          usermanageExportUser(values)
            .then((res) => {
              ...
              res.fileData.forEach((u, index) => {
                 let data = res.fileData[index].replace(/ /g, ',')
                   data = data.split(',')
                    if (index === 0) {
            ...
            // res.fileData.forEach((u, index) => {
            //      let data = res.fileData[index].replace(/ /g, ',')
            //     if (index < res.fileData.length - 1) {
            //         data = data + '\n'
            //     }
            //         this.fileData.push(data)
            // })
            // console.log(this.fileData)
            //   const url = this.genUrl(this.fileData, this.header) // {}指的是表头，res.data.data.workhour_csv_data是后台返回来的数据
    ...
    ouSort (record) {
      record.forEach((item) => {
        ...
        ouSortvalue = (ouSortvalue + 'ou=Citrix,dc=cloud,,dc=com').replace(/(^\s*)/g, '')
        const obj = {
          name: item.title,
          value: ouSortvalue,
          key: item.key
