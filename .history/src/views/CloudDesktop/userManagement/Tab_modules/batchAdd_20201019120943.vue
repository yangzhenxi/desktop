    ...
    methods: {
        ...
        async handleSubmit () {
            ...
            if (this.IsvalidatorSuccess) {
                ...
                let boole = true
                let u = 0
                console.log(this.dataArr.slice(0, 30))
                console.log(this.dataArr.slice(31, 60))
                console.log(this.dataArr.slice(61, 90))
                console.log(this.dataArr.slice(91, 120))
                console.log(this.dataArr.slice(121, 150))
                console.log(this.dataArr.slice(151, 201))
                for (let i = 0; i < (this.dataArr.length - 30); i = i + 30) {
                    let fileData = []
                    if (Math.floor((this.dataArr.length - u * 30) / 30) > 1 && i === 0) {
                        fileData = this.dataArr.slice((u * 30), (u + 1) * 30 + 1)
                    } else if (Math.floor((this.dataArr.length - u * 30) / 30) > 1) {
                        const arr = this.dataArr.slice((u * 30) + 1, (u + 1) * 30)
                        fileData = [this.dataArr[0], ...arr]
                    } else {
                        const arr = this.dataArr.slice((u * 30) + 1)
                        fileData = [this.dataArr[0], ...arr]
                    }
                    const obj = {
                        filename: 'filename' + u + '.csv',
                        filetype: this.filetype,
                        fileData: fileData,
                        ou: this.record.dn
                    }
                    u = u + 1
                    apis.push(usermanageBatchAddUser(obj))
                }
