    ...
    methods: {
        ...
        beforeUpload (file) {
            ...
            reader.onloadend = evt => {
                ...
                this.ErrorIndex = ['用户名', '姓名', '密码', '是否下次登录修改密码', '是否不能修改密码', '是否禁用账户', '是否密码永不过期']
            ...
            this.AllData.forEach((u, index) => {
                if (index > 0 && index < this.AllData.length) {
                    if (u !== '') {
                        ...
                        row.forEach((o, sum) => {
                            if (this.titleNameChange(this.ErrorIndex[sum], o)) {
                                this.$message.error(
                                    '第' +
                                        index +
                                        '行，' +
                                        this.ErrorIndex[sum] +
                                        this.titleNameChange(
                                            this.ErrorIndex[sum],
                                            o
                                        )
                                )
        ...
        titleNameChange (key, val) {
            ...
            switch (key) {
                ...
                case '是否不能修改密码':
                    return this.isAir(val, '是否不能修改密码') && this.pasNoExpired(val)
                     
                case '是否禁用账户':
                    this.isAir(val, '是否禁用账户')
                    return this.whether(val)
                case '是否密码永不过期':
                    this.isAir(val, '是否密码永不过期')
                    return this.pasNoExpired(val)
                case '是否下次登录修改密码':
                    this.isAir(val, '是否下次登录修改密码')
