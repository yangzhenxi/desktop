  ...
  methods: {
    ...
    beforeUpload (file) {
      ...
      reader.onloadend = (evt) => {
        ...
        dataArr[0] = ['用户名(英文加数字,长度为2-8), 姓名(长度为2 - 8), 密码(长度为7 - 16), 是否下次登录修改密码(是或否), 是否不能修改密码(是或否), 是否禁用账户(是或否), 是否密码永不过期(是或否)']
      ...
      this.AllData.forEach((u, index) => {
        if (index > 0 && index < this.AllData.length) {
          if (u !== '') {
            ...
            row.forEach((o, sum) => {
              if (this.titleNameChange(this.ErrorArr[sum], o) === 1) {
                this.$message.error('第' + index + '行，' + this.ErrorArr[sum] + '格式错误')
                this.IsvalidatorSuccess = false
              }
            })
          }
