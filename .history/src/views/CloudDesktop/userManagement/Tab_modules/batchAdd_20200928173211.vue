    ...
    beforeUpload (file) {
      ...
      reader.onloadend = (evt) => {
        const res = evt.target.result
         this.dataArr = res.split('\n')
        this.dataArr.forEach((u, index) => {
          if (index < this.dataArr.length) {
            const Arr = this.dataArr[index].split(',')
            Arr.forEach((u, index) => {
              if (u.length === 0) this.ErrorIndex.push(index)
              if (index === Arr.length - 1 && u.length === 1) this.ErrorIndex.push(index)
            })
            let add = this.dataArr[index].replace(new RegExp(',', 'g'), ' ')
            if (index === 0) this.ErrorArr = Arr
            if (index !== this.dataArr.length - 2) add += '\n'
            if (add.length > 0) {
                const Base64 = require('js-base64').Base64
                this.this.dataArr.push(Base64.encode(add)) // 加密
            }
          }
        })
      }
...
/deep/i.anticon.anticon-paper-clip,/deep/i.anticon.anticon-delete,/deep/span.ant-upload-list-item-name  {
    color: white;
}
</style>
