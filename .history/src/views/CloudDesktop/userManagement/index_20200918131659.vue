          ...
          <a-row>
            ...
            <a-col :span="14">
              <a-button
                type="primary"
                shape="circle"
                @click="$refs.TreeAdd.add(queryParam)">
                <m-icon
                  type="yonghutianjia"
              ...
              <a-button
                ...
                @click="$refs.TreeEdit.Edit(queryParam)">
                <m-icon
                  type="xiugai"
                  class="icon xiugai" />
              </a-button>
        ...
        <a-card>
          <m-table
            ...
            <template slot="accountControl" slot-scope="text">
              <a-badge :number-style="{backgroundColor: toDict(text.DisableCount,'UM_STATE').color}" :count="text.DisableCount | convert('UM_STATE')" />
            </template>
            <template slot="whenCreated" slot-scope="text">
              {{ text | convert('unix','YYYY-HH-DD') }}
            </template>
            <template slot="lastLogon" slot-scope="text">
              {{ text | convert('unix','YYYY-HH-DD') }}
            </template>
            <template
              slot="action"
    ...
    return {
      ...
      loadData: async (parameter) => {
            ...
            if (!this.queryParam.name) {
                this.queryParam.name = 'ou=' + OuList.ou[0].children[0].children[0].name + ',ou=Users,ou=Citrix,dc=test,dc=com'
            }
            this.OuList = []
            this.Recursion(OuList.ou[0].children[0].children[0])
            this.loading = false
  ...
  methods: {
    ...
    changecheckedval (item) {
      ...
      if (item.eventKey.search('-') !== -1) {
        ...
        this.queryParam.name = this.queryParam.name + ',ou=Users,ou=Citrix,dc=test,dc=com'
        this.$refs.table.refresh()
        return true
      }
      this.queryParam.name = 'ou=' + item.title + ',ou=Users,ou=Citrix,dc=test,dc=com'
      this.$refs.table.refresh()
    },
    // 递归
