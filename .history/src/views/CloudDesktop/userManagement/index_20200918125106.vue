  ...
  data () {
    return {
      ...
      loadData: async (parameter) => {
            ...
            console.log(OuList.ou[0].children[0].children[0].name)
            this.queryParam.name = 'ou=' + OuList.ou[0].children[0].children[0].name + ',ou=Citrix,dc=test,dc=com'
            this.Recursion(OuList.ou[0].children[0].children)
            this.loading = false
            console.log(this.queryParam)
        // let data = this.deepGet(await usermanageListuser(this.queryParam), [])
    let data = []
if (data === undefined) {
          data = []
        }
  ...
  methods: {
    ...
    async getOuList () {
      const OuList = await usermanageListOU()
      this.queryParam.name = 'ou=' + OuList.ou[0].children[0].children.name + ',ou=Citrix,dc=test,dc=com'
      this.Recursion(OuList.ou[0].children[0].children)
      this.loading = false
    },
    Delete (record) {
