...
export default {
  ...
  methods: {
    ...
    // 删除OU
    TreeDelete () {
      if (this.treeeventObj.dataRef.children.length !== 0) {
    ...
    changecheckedval (item) {
      ...
      this.checkedval.push(item.eventKey)
    ...
    //   this.ouList = this.ouList.sort((e1, e2) => {
    //     const s1 = deepGet(e1, 'title')
    //     const s2 = deepGet(e2, 'title')
    //     if (s1 > s2 || (s1 && !s2)) {
    //       return 1
    //     } else if (s1 < s2 || (!s1 && s2)) {
    //       return -1
    ...
    sotsArr (e1, e2) {
      ...
      const s2 = deepGet(e2, 'title')
      if (s1 > s2 || (s1 && !s2)) {
          console.log(e1.children.length)
          if (e1.children.length > 1) {
              e1.sort(this.sotsArr)
          }
        return 1
      } else if (s1 < s2 || (!s1 && s2)) {
          console.log(e2.children.length)
          if (e2.children.length > 1) {
              e2.sort(this.sotsArr)
          }
        return -1
      }
    }
