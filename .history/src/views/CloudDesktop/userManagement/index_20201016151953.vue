...
export default {
            ...
            loadData: async parameter => {
                try {
                    ...
                    if (!this.queryParam.name) {
                        ...
                        //     this.treeeventObj.OU =
                        //         OuList.ou[0].children[0].children[0].name

                        //     this.queryParam.name =
                        //         'ou=' +
                        //         OuList.ou[0].children[0].children[0].name +
                        //         ',ou=Users,ou=Citrix,dc=cloud,dc=com'
                        // } else {
                        //     this.treeeventObj = []
                        //     this.queryParam.name =
                        //         'ou=Users,ou=Citrix,dc=cloud,dc=com'
                    ...
                    const data = this.deepGet(await usermanageListuser({ dn: this.record.dn }), 'list', [])
                    return {
                        data,
    ...
    methods: {
        ...
        // 删除OU
        TreeDelete () {
            if (this.record.userCount !== 0) {
                this.$message.info('当前选中的OU下还有子Ou暂不能删除')
                return false
            }
            this.$confirm({
                title: '是否要删除' + this.record.title + '组?',
                content: '点击确定即可删除当前组',
                onOk: () => {
        ...
        changecheckedval (item) {
            ...
            this.queryParam = {}
            this.queryParam.name =
                item.OU + ',ou=Users,ou=Citrix,dc=cloud,dc=com'
            this.$refs.table.refresh()
        },
        // 递归
