...
import { MTable, MIcon, Ellipsis } from '@/components'
...
export default {
    ...
    data () {
        return {
            ...
            loadData: async parameter => {
                try {
					const OuList = this.deepGet(this.deepGet(await usermanageListOU(), 'ou', [])[0], 'children', [])[0]
                    this.loading = false
                    this.ouList = []
                    this.TAbleData = []
					OuList.children = OuList.children.sort(this.sotsArr)
                    if (OuList.children.length > 0 && !this.record.dn) {
						this.selectedKeys = ['0']
                        this.record = OuList.children[0]
					} else if (OuList.children.length === 0) {
						this.record = OuList
					}
                    this.OuAllName = []
                    this.Recursion(OuList.children)
					let data = []
                    this.record.dn === 'ou=Users,ou=Citrix,dc=cloud,dc=com' ? data = this.deepGet(await usermanageAllUser({ name: this.record.dn }), 'list', []) : data = this.deepGet(await usermanageListuser({ name: this.record.dn }), 'list', [])
                    data.forEach(u => {
	...
	computed: {
		disabled: function () {
			console.log(this.record.dn !== 'ou=Users,ou=Citrix,dc=cloud,dc=com'){}
			if(this.record)
			return true
		}
	},
    ...
    methods: {
        ...
        BatchDelUser () {
            const BatchDel = {}
            BatchDel.baseDN = this.deleteTabName.map(u => { return u.baseDN })
