<template>
  <page-header-wrapper :title="false">
    <a-row :gutter="16">
      <a-col ref="left"
             :span="6">
        <a-card style="height:842px;"
                :loading="loading">
          <empty v-if="!ouList.length>0"
                 :isEmpty="!ouList.length>0" />
          <a-row v-else>
            <a-col :span="10">
              <a-icon class="icon"
                      type="user" />
      ...
      <a-col ref="right"
        ...
        <div class="head">
          <div class="btns">
            <a-button @click="$refs.TabAdd.add(treeeventObj)"
                      type="primary">
              新增
            </a-button>
            <a-button type="primary"
            ...
            <a-button @click="$refs.BatchTabAdd.BatchAdd(treeeventObj)"
                      type="primary"
                      style="margin-left:10px">
              批量新增
            </a-button>
            <a-button @click="BatchDelUser()"
                      type="danger"
                      style="margin-left:10px">
              批量删除
            </a-button>
        ...
        <a-card>
          <m-table ref="table"
            ...
            <template slot="accountControl"
                      slot-scope="text">
              <a-badge :number-style="{backgroundColor: toDict(text.DisableCount,'UM_STATE').color}"
...
export default {
    ...
    width: 200
      ...
      loadData: async (parameter) => {
        try {
                ...
                this.selectedKeys = ['0']
                if (OuList.ou[0].children[0].children[0]) {
                    this.treeeventObj.OU = OuList.ou[0].children[0].children[0].name

                    this.queryParam.name =
                    'ou=' + OuList.ou[0].children[0].children[0].name + ',ou=Users,ou=Citrix,dc=cloud,dc=com'
                } else {
                    this.treeeventObj = []
                    this.queryParam.name = 'ou=Users,ou=Citrix,dc=cloud,dc=com'
                }
                }
  ...
  methods: {
    ...
    changecheckedval (item) {
      ...
      this.queryParam = {}
      this.queryParam.name = item.OU + ',ou=Users,ou=Citrix,dc=cloud,dc=com'
      this.$refs.table.refresh()
    },
    // 递归
