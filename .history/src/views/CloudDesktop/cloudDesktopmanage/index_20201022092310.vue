    ...
    methods: {
        ...
        async GetTask () {
            for (let i = 0; i < this.DesktopCreatingId.length; i++) {
                await CloudDesktopTaskGet({ id: this.DesktopCreatingId[i] })
            }
            const apis = []
            this.DesktopCreatingId.forEach(u => {
                apis.push(CloudDesktopTaskGet({ id: u }))
            })
            Promise.all(apis).then(result => {
                result.forEach(u => {
                    const i = this.deepGet(u, 'data', {})
                    i.progress = {
                            input: 10,
                            output: 20,
                            cpuIdle: parseInt(u.progress / 100)
                    }
                    this.DesktopCreating.push(i)
                })
            })
            const AllTPlist = [...this.DesktopCreating, ...this.DesktopSuccess, ...this.DesktopFail]
            console.log(AllTPlist)
            console.log(this.DesktopCreating)
