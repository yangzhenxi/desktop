              ...
              <a-card style="height:811px">
                <a-row :gutter="[36,24]">
                  ...
                  <div v-for="(i,index) in TPList" :key="index">
                    ...
                    <!-- 删除任务进度 -->
                    <a-col
                      :span="8"v-else-if="i.state ==='"DESKTOP_STATE_DELETING"' " >
                      <a-row
                        class="modules progress"
                        type="flex"
                        :gutter="16">
                      ...
                      </a-row></a-col></div></a-row></a-card></a-spin></div></div></a-spin></a-col></div></a-spin></a-row></a-spin></a-col></div></a-card></a-row></div></a-col></div></a-spin></a-card></a-row></a-col></a-row>
    </div></a-spin></a-card></a-col>
    <!-- drawer -->
    <a-drawer
      placement="right"
      :closable="false"
        ...
        async GetDeleteTask () {
            for (let i = 0; i < this.DesktopDeleting.length; i++) {
                await CloudDesktopTaskGet({ id: this.DesktopDeleting[i].task_id }).then(result => {
                    const res = this.deepGet(result, 'data', {})
                    this.DesktopDeleting.forEach(u => {
                        if (res.id === u.task_id) {
                            u.progress = res.progress
                        }
                    })
                })
                console.log(this.DesktopDeleting)
