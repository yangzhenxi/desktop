        ...
        this.$nextTick(() => {
            if (res.data.OffPeakBufferSizePercent > 0) {
                this.form.setFieldsValue({
                    peak: true,
                    BufferSizePercent: this.formValue.namingScheme,
                    DisconnectAction: this.formValue.group,
                    DisconnectTimeout: this.formValue.template,
                    LogOffAction: this.formValue.version,
                    LogOffTimeout: this.formValue.NamingSchemeType
                })
            } else {
