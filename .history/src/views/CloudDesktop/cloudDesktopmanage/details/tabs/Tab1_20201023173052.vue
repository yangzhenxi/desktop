  ...
  data () {
    return {
      ...
      loadData: async (parameter) => {
            ...
            this.ISstate ? const data = this.rawdata : const data = this.deepGet(await CloudDesktopMachineList({ desktop_id: this.datasource.id }), 'data', [])
        return {
          data,
    ...
    async getTab1 (Ids) {
        for (let i = 0; i < Ids.length; i++) {
            await CloudDesktopTaskGet({ id: Ids[i] }).then(res => {
                const state = this.deepGet(res, 'state', '')
                if (state === 'SYSTEM_TASK_STATE_RUNNING') {
                    // 执行
                    this
                }
            })
