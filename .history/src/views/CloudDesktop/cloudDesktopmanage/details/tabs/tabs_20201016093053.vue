        ...
        this.$confirm({
          ...
          onOk: () => {
            return new Promise(async (resolve, reject) => {
                const power = this.checked.find(u => {
                    if (u.PowerState === 'Off') {
                        return u.HostedMachineName
                    }
                })
                if (!power) {
                    this.$message.success('当前选中的桌面都已开机')
                    return false
                }
              const obj = {
    ...
    DesktopOff () {
      if (this.checked.length > 0) {
        this.$confirm({
          ...
          onOk: () => {
            return new Promise(async (resolve, reject) => {
                const power = this.checked.forEach(u => {
                    if (u.PowerState === 'On') {
                        return u.HostedMachineName
                    }
                })
                if (!power) {
                    this.$message.success('当前选中的桌面都已关机')
                    return false
                }
              const obj = {
                DesktopId: this.datasource.id,
    ...
    DesktopReSet () {
      if (this.checked.length > 0) {
        this.$confirm({
          ...
          onOk: () => {
            return new Promise(async (resolve, reject) => {
             const power = this.checked.find(u => u.HostedMachineName)
              const obj = {
                DesktopId: this.datasource.id,
                HostedMachineName: power,
                power: 'Reboot'
