...
export default {
    ...
    methods: {
        async Add (record) {
          ...
          const Users = this.deepGet(this.deepGet(await CloudDesktopPolicyList({ desktop_id: this.record.id }), 'data', []), 'users', [])
          ...
          this.oldSid = Group.find(u => u.sid)
          this.oldSid = Users.find(u => u.sid)
          this.loading = false
        },
        handleSubmit () {
