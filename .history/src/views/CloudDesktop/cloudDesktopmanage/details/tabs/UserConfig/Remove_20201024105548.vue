    ...
    handleSubmit () {
      this.form.validateFields(async (errors, values) => {
        if (!errors) {
            ...
            let RemoveSids = []
            if (values.Usersids && values.Groupsisd) {
                 RemoveSids = [...values.Usersids, ...values.Groupsisd]
            } else if (values.Usersids) {
                RemoveSids = [...values.Usersids]
            } else if (values.Groupsisd) {
                RemoveSids = [...values.Groupsisd]
            }
            console.log(RemoveSids)
            console.log(this.sids)
            const obj = {
                desktop_id: this.record.id,
                sids: [...this.sids].filter(u => [...RemoveSids].every(y => y !== u.sid))
            }
