...
export default {
    ...
    methods: {
        async Add (record) {
            ...
            this.Group = this.deepGet(this.deepGet(await CloudDesktopPolicyList({ desktop_id: this.record.id }), 'data', []), 'groups', []).map(u => u.sid)
            this.UsersSid = this.deepGet(this.deepGet(await CloudDesktopPolicyList({ desktop_id: this.record.id }), 'data', []), 'users', []).map(u => u.sid)
        ...
        handleSubmit () {
            ...
            this.form.validateFields(async (errors, values) => {
                if (!errors) {
                    ...
                    obj.sids = this.Group
                    console.log(obj)
                    obj.sids = [...this.UsersSid, ...obj.sids]
