  ...
  <a-card>
    <a-tabs
      ...
      <a-tab-pane
        ...
        tab="用户配置"
        force-render>
        <tab2 ref="tab2" :datasource="result"></tab2>
      </a-tab-pane>
      <a-tab-pane
        key="3"
        tab="会话列表"
        force-render>
        <tab3 ref="tab3" :datasource="result" @chaned="chaned"></tab3>
      </a-tab-pane>
      <a-tab-pane
        key="4"
        tab="电源策略"
        force-render>
        <tab4 ref="tab4" :datasource="result"></tab4>
      </a-tab-pane>
...
import {
  ...
  CloudDesktopSessionList,
  CloudDesktopPolicyList,
  CloudDesktopGroupGet
  ...
  methods: {
    ...
    async changeActive (val) {
      ...
      if (val === '1') {
        ...
        this.result.push(deepGet(await CloudDesktopPolicyList({ desktop_id: this.datasource.id }), 'data', []))
        ...
        this.result = deepGet(await CloudDesktopSessionList({ desktop_id: this.datasource.id }), 'data', [])
        this.$refs.tab3.$refs.table.refresh()
      } else if (val === '4') {
        this.isSearchVisible = false
        this.isActiveVisible = false
        this.isSetColumns = false
        this.result.push(deepGet(await CloudDesktopGroupGet({ desktop_id: this.datasource.id }), 'data', []))
    ...
    async GetTabData (tab) {
        switch (tab) {
            ...
            case '3':
                ...
                break
            case '4':
                this.result = deepGet(await CloudDesktopGroupGet({ desktop_id: this.datasource.id }), 'data', [])
                break
        }
