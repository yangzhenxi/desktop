    ...
    methods: {
        ...
        async GetTPList (item, Creating) {
            ...
            try {
                ...
                this.DesktopCreatingId.length > 0 || this.DesktopDeleting.length > 0 ? this.GetTask() : this.IsDesktopCreating = false
        ...
        showDrawer (i) {
            this.$nextTick(async () => {
                ...
                if (!i.steps) {
                    try {
                        const result = this.deepGet(await CloudDesktopTaskGet({ id: this.DesktopFailId[i] }), 'data', [])
                    } catch (err) {
