...
export default {
    ...
    methods: {
        ...mapMutations(['SET_FORMVALUE', 'SET_NETWORKS', 'SET_HOSTINGUNIT']),
        ...
        async getHostList () {
			...
			try {
				this.Host = deepGet(await CloudDesktopHostList(),'data',[]
					this.Host = deepGet(res, 'data', [])
					this.SET_HOSTINGUNIT(this.Host)
        ...
        nextStep () {
            ...
            validateFields((err, values) => {
                if (!err) {
                    ...
                    this.SET_NETWORKS(this.networks)
                    setTimeout(function () {
                        that.loading = false
                        that.$emit('nextStep')
                    }, 500)
