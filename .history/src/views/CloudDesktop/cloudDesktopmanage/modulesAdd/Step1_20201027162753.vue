      ...
      <a-form
        ...
        <a-form-item label="桌面名称">
          <a-input
            autocomplete="off"
            placeholder="请输入桌面名称"
            v-decorator="['name', { rules: [{required: true, message: '请输入桌面名称'},{min:1,max:13,message:'桌面长度在1-13个字符'},{validator}],validateFirst: true }]" />
        </a-form-item>
        <a-form-item
          label="名称前缀"
        >
...
<script>
import { namingScheme } from '@/utils/validator'
import { deepGet } from '@/utils/util'
import { mapState, mapMutations } from 'vuex'
import { CloudDesktopOUList, CloudDesktopTestName } from '@/api/CloudDesktop/CloudDesktop'
  ...
  methods: {
    ...
    async validator (rule, value, callback) {
        ...
        if (!pattern.test(value)) {
            callback(new Error('桌面名称不能有特殊字符，且不能数字开头'))
            return false
        }
