              ...
              <a-card style="height:811px">
                <a-row :gutter="[36,24]">
                  ...
                  <div v-for="(i,index) in TPList" :key="index">
                        ...
                        <a-col :span="8">
                          <a-row style="margin-top: 35px;">
                            <m-icon
                              class="icon"
                              type="windowsfuwuqi" />
                            <div class="icon_title">云桌面创建中</div>
                          </a-row>
                        </a-col>
                        <a-col
                          class="blurry"
                    ...
                    >
                      <a-timeline>
                        <a-timeline-item v-for="i in DesktopCreatingDrawer.steps" :key="i.id" :color="toDict(i.state,'C_D_DESKTOP_TASK_ATSTE').color">
                          <p>{{ i.name }} </p>
                          <p v-if="i.state === 'SYSTEM_TASK_STATE_RUNNING'" style="display:flex;justify-content: space-between">
                            <span>{{ i.state | convert('C_D_DESKTOP_TASK_ATSTE') }}</span>
                            <a-progress :percent="i.progress" status="active" style="width:80%;" />
                          </p>
                          <p v-else>{{ i.state | convert('C_D_DESKTOP_TASK_ATSTE') }} </p>
                        </a-timeline-item>
                      </a-timeline>
                    </a-drawer>
                  </div>
        ...
        async GetTask () {
            for (let i = 0; i < this.DesktopCreatingId.length; i++) {
                await CloudDesktopTaskGet({ id: this.DesktopCreatingId[i] }).then(result => {
                    const i = this.deepGet(result, 'data', {})
                    i.progress = {
                            input: 10,
                            output: 20,
                            cpuIdle: parseInt(result.progress / 100)
                    }
                    this.DesktopCreating.push(i)
                })
            }
