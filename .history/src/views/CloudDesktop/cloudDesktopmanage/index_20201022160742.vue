            ...
            <a-spin :spinning="spinning">
              <a-card style="height:811px">
                ...
                <a-pagination
                  ...
                  :total="AllTPlist.length"
                  :show-total="total => `共 ${AllTPlist.length} 条数据`"
                  :page-size="8"
                  @change="changePage"
                  :default-current="1"/>
              </a-card>
    ...
    methods: {
        ...
        async GetTPList (item, Creating) {
            ...
            try {
                ...
                if (this.DesktopFailId.length > 0 && Creating !== 'Creating') {
                    this.GetFail()
                }
                if (this.DesktopCreatingId.length === 0 && this.DesktopFailId.length === 0) {
                    this.AllTPlist = [...this.DesktopCreating, ...this.DesktopSuccess, ...this.DesktopFail]
                    this.TPList = this.AllTPlist.slice(0, 8)
                    console.log(this.AllTPlist)
                }
                this.settime()
            } catch {
                this.spinning = false
