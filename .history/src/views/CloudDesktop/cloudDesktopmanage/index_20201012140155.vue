              ...
              <a-col :span="10">
                <a-row style="height:32px;">
                  ...
                  <a-col :span="14">
                    <span
                      class="All"
                      style="font-size:15px;">所有桌面组</span>
                  </a-col>
                </a-row>
              </a-col>
              <a-col
                ...
                <a-button
                  ...
                  type="primary"
                  shape="circle"
                  @click="$refs.TreeAdd.Add(selectedKeys)">
                  <m-icon
                    type="yonghutianjia"
                    class="icon" />
                </a-button>
            ...
            <div v-if="IsAllmodule">
              ...
              <a-card>
                ...
                <a-pagination
                  style="float:right"
                  :total="5"
                  :show-total="total => `共 `+ '1' +` 条数据`"
                  :page-size="9"
                  @change="changePage"
                  :default-current="1" />
              </a-card>
            </div>
            <singlemodules ref="Single"></singlemodules>
          </div>
        </a-col>
      </a-row>
      <tree-add
        ref="TreeAdd"
        @ok="getCloudList"></tree-add>
      <tree-edit
        ref="TreeEdit"
        @ok="getCloudList"></tree-edit>
      <module-add
        ref="ModulesAdd"
        @ok="getCloudList"></module-add>
    </a-spin>
  </page-header-wrapper>
</template>
...
import { CloudDesktopTPList, CloudDesktopOUList } from '@/api/CloudDesktop/CloudDesktop'
...
export default {
  ...
  methods: {
    ...
    async GetTPList () {
      const obj = {
        dn: 'OU=Win7,OU=Computers,OU=Citrix,DC=cloud,DC=com',
        page: 0,
        size: 20
      }
      const res = this.deepGet(await CloudDesktopTPList(obj), 'data', [])
      res.forEach((u) => {
        if (u.state === 'DesktopCreating') {
          this.Creating.push(u)
        } else if (u.state === 'DesktopSuccess') {
          this.TPList.push(u)
        } else if (u.state === 'DesktopFail') {
          this.TPList.push(u)
        }
      })
    },
