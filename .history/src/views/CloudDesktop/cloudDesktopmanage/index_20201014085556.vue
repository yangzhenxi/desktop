            ...
            <a-spin :spinning="spinning">
              <a-card style="height:811px">
                ...
                <a-pagination
                  ...
                  :total="5"
                  :show-total="total => `共 ${TPList.length + Creating.length} 条数据`"
                  :page-size="9"
                  @change="changePage"
                  :default-current="1" />
              </a-card>
...
// import { sotsArr } from '@/utils/util'
export default {
    mixins: [mixin],
    components: {
        MIcon,
    ...
    methods: {
        ...
        async getOu (item) {
            ...
            this.validatorName = []
            debugger
            const OuList = this.deepGet(this.deepGet(await CloudDesktopOUList(), 'ou')[0], 'children', '').sort(this.sotsArr)
            console.log(OuList)
                ...
                res.forEach(u => {
                    if (u.state === 'DesktopCreating') {
                        u.progress = {
                            input: 10,
                            output: 20,
                            cpuIdle: parseInt(u.task.progress / 100)
                        }
