<template>
  <page-header-wrapper :title="false">
              ...
              <a-row style="height:32px;">
                ...
                <a-col :span="14">
                  <div
                    class="All"
                    @click="close"
                    style="font-size:15px;margin-top:5px;">所有模版组</div>
                </a-col>
              </a-row>
            </a-col>
            <a-col
              ...
              <div v-if="IsAllmodule">
                ...
                <a-card style="height:730px;">
                  <a-row :gutter="[36,24]">
                    ...
                    <div v-for="(i,index) in showTPpageList" :key="index">
                      <!-- 要创建任务的数据 -->
                      <a-col :span="8" v-if="i.data">
                        <a-row
    ...
    <x-manageadd
      ref="ManageAdd"
      @ok="getOUGroup(record)"></x-manageadd>
    <x-managerelease
      ref="ManageRelease"
    ...
    methods: {
        ...
        async getTP () {
            ...
            const [
                ...
                [
                    DesktopTemplateAllList({ group: parseInt(this.record.id) }),
                    DesktopTemplateImageList(),
                    DesktopTemplateDataCenterList()
                ].map(u => u.catch(e => console.log(e)))
            )
            await this.gettask()
            this.AllTask.length > 0 ? this.Task() : clearInterval(this.timer)
            ...
            this.AllTPpageList.length > 8 ? this.showTPpageList = this.AllTPpageList.slice(0, 8) : this.showTPpageList = this.AllTPpageList
            this.spinning = false
        },
        // 用户点击模版组切换
        ...
        // 模版删除
       async  Delete (i) {
            const res = this.deepGet(await DesktopGetTemplate({ id: i.id }), 'data', [])
            if (res.is_running === '关机' || res.is_running === '挂起') {
                this.$confirm({
                ...
                this.$message.info('当前模版正在运行，请关机在尝试')
            }
        },
        // 显示详细模版信息
        Single () {
        ...
        async setTimedTask () {
            if (this.AllTask.length > 0 && this.AllTask[0] !== ':') {
                const obj = {}
                obj.task = this.AllTask.map(u => u.task_ref)
        ...
        async isAllTask () {
            if (this.AllTask.length > 0) {
                const TPList = await DesktopTemplateAllList({ group: parseInt(this.record.id) })
                this.AllTPpageList = deepGet(TPList, 'data', [])
                const progress = []
                this.Allprogress.forEach(u => {
                    if (u.group === this.selectedKeys[0] && u.progress_bar !== 100) {
                            u.data = {
                                input: 10,
                                output: 20,
        ...
        // 获取任务
        async gettask () {
            const Task = await DesktopTemplateGetAllTask()
            this.AllTask = this.deepGet(Task, 'data', [])
        },
        // 返回
        back () {
            this.$refs.Single.OffSingle()
