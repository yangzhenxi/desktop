...
export default {
  ...
  methods: {
      ...
      async getTP () {
            DesktopTemplateImageList(),
          const [AllTask, GetTP,ImageList] = (await Promise.all([DesktopTemplateGetAllTask(), DesktopTemplateAllList({ group: parseInt(this.record.id) }),].map((u) => u.catch(e => console.log(1)))))
          if (AllTask.length > 0) {
