...
export default {
  ...
  methods: {
    ...
    // 获取数据中心和系统镜像接口
        const [DataCenterList, ImageList, GroupList] = (await Promise.all([DesktopTemplateDataCenterList(), DesktopTemplateImageList(), DesktopTemplateGroupList()].map(u => u.catch(e => e)))).map(u => deepGet(u, [], []))
        this.AddData.ImageList = ImageList
        this.AddData.DataCenterList = DataCenterList
        await DesktopTemplateAllList().then((GroupList) => {
            this.TPAllList = res.data
        })
      if (res.data) {
        this.selectedKeys.push(res.data.children[0].id)
        this.TPname = res.data.children[0].name
