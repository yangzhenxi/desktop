            ...
            <a-row class="tree_head">
              ...
              <a-col :span="14" class="flex_end">
                <a-button
                  ...
                  type="primary"
                  shape="circle"
                  @click="$refs.Add.Add(record,TPgroupNameList)">
                  <m-icon
                    type="yonghutianjia"
                    class="icon" />
                </a-button>
                <a-button
                  ...
                  @click="$refs.Edit.Edit(record,TPgroupNameList)">
                  <m-icon
                    type="xiugai"
                    class="icon" />
                </a-button>
              ...
              <div v-if="IsAllmodule">
                ...
                <a-card style="height:730px;">
                  ...
                  <a-pagination
                    ...
                    :show-total="total => `共 ${TPpageList.length + progress.length} 条数据`"
                    :page-size="9"
                    @change="changePage"
                    :default-current="1" />
                </a-card>
...
export default {
  ...
  data () {
    return {
      record: {},
      TPgroupList: [], // 模版组列表
      TPgroupNameList: [], // 模版组名称  验证重名 不存在children
      disabledAdd: false,
      disabledEdit: false,
  ...
  methods: {
        ...mapMutations(['SET_DATACENTERLIST', 'SET_IMAGELIST', 'SET_TPGROUPLIST']),
      async getOUGroup () {
            this.TPgroupList = []
          const GroupList = deepGet(await DesktopTemplateGroupList(), 'data', [])
            if (GroupList) {
      ...
      async getTP (item) {
          const [AllTask, GetTP, ImageList, DataCenterList] = (await Promise.all([DesktopTemplateGetAllTask(), DesktopTemplateAllList({ group: parseInt(this.record.id) }), DesktopTemplateImageList(), DesktopTemplateDataCenterList()].map((u) => u.catch(e => console.log(1)))))
          if (deepGet(AllTask, 'data', []).length > 0) {
              this.AllTask = deepGet(AllTask, 'data', [])
              this.Task()
          }
    ...
    TreeDelete () {
      ...
      this.$confirm({
        ...
        content: '点击确定即可删除当前组',
        onOk: async () => {
          await DesktopTemplateGroupDelete({ id: this.record.key }).then((res) => {
    ...
    Task () {
      ...
      if (this.AllTask.length > 0) {
        this.timer = setInterval(() => {
            this.setTimedTask()
            if (!this.timeTaskcatch) clearInterval(this.timer)
        }, 3000)
      }
    },
    // 定时器任务
    async setTimedTask () {
      if (this.AllTask.length > 0 && this.AllTask[0] !== ':') {
        ...
        obj.task = this.AllTask.map((u) => u.taskRef)
        await DesktopTemplateCheckTask(obj).then((res) => {
          this.Allprogress = [...this.deepGet(res, 'progress', [])]
        }).catch(e => {
    ...
    isAllTask () {
      ...
      if (this.AllTask.length > 0) {
        this.Allprogress.forEach((u) => {
          if (u.group === this.selectedKeys[0]) {
              u.data = {
                input: 10,
                output: 20,
                cpuIdle: u.progressBar
              }
            this.progress.push(u)
          }
          if (u.msg === '任务全部完成' || this.AllTask.length !== this.Allprogress.length) {
            this.progress = []
            this.getTP()
          }
        })
    ...
    closeTask (task) {
      this.$confirm({
        ...
        onOk: () => {
          ...
          return new Promise(async (resolve, reject) => {
            try {
              await DesktopTemplateCheckTask({ task: [task.task] }).then((res) => {
                if (res.progress[0].progressBar >= 90) {
                  this.$message.info('该任务即将完成，无法取消')
                  resolve()
                  return true
                } else {
