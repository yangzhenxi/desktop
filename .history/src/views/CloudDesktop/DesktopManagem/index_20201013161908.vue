...
export default {
  ...
  methods: {
      ...
      async getTP () {
          const [AllTask, GetTP] = (await Promise.all([DesktopTemplateGetAllTask(), DesktopGetTemplate({ id: this.record.id })].map((u) => u.catch(e => console.log(1)))))
          if (AllTask.length > 0) {
              this.AllTask = AllTask
              this.Task()
          }
          console.log(this.GetTP)
