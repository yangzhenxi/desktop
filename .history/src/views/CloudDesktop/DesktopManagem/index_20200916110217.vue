...
export default {
  ...
  methods: {
    ...
    // 获取数据中心和系统镜像接口
        const [DataCenterList, ImageList, GroupList] = (await Promise.all([DesktopTemplateDataCenterList(), DesktopTemplateImageList(), DesktopTemplateGroupList()].map(u => u.catch(e => e)))).map(u => deepGet(u, 'data', []))
