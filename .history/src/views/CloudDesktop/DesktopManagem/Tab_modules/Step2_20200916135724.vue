...
export default {
  ...
  methods: {
      // 解析datacenterList的数据
      getdataCenter () {
          console.log(this.form.getFieldsValue(['dataCenter']).dataCenter)
        this.data.DataCenterList.forEach(u => {
            console.log(u.name)
            if (u.name === this.form.getFieldsValue(['dataCenter']).dataCenter) {
                this.clustersHosts = u.clusters
                this.clustersHosts.push(...u.hosts)
            }
        })

        console.log(this.clustersHosts)
      },
    nextStep () {
      const that = this
      const {
