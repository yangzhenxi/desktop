    ...
    <a-form
      ...
      <a-form-item
        ...
        <a-select
          placeholder="请选择资源池"
          @change="getdataCenter"
          v-decorator="['resourcePools', { rules: [{required: true, message: '请选择资源池'}] }]">
          <a-select-option v-for="(i,index) in resourcePools" :key="index" :value="i.name">{{ i.name }}</a-select-option>
        </a-select>
      </a-form-item>
      <a-form-item
        label="网络"
      ...
      validateFields((err, values) => {
        if (!err) {
          ...
          that.timer = setTimeout(function () {
            that.loading = false
            that.$emit('nextStep', this.dataStores)
          }, 1500)
        } else {
          that.loading = false
        }
