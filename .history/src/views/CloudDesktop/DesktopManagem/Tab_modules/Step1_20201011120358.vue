  ...
  <div class="step1">
    <a-form :form="form"
      ...
      <a-form-item label="系统镜像"
                   :labelCol="labelCol"
                   :wrapperCol="wrapperCol">
        <a-select placeholder="请选择系统镜像"
                  v-decorator="['image', { rules: [{required: true, message: '请选择系统镜像'}] }]">
          <a-select-option v-for="(i,index) in data.ImageList"
                           :key="index"
      ...
      <a-form-item label="软件信息"
                   :labelCol="labelCol"
                   :wrapperCol="wrapperCol">
        <a-textarea placeholder="请输入软件信息"
                    :rows="4"
...
export default {
    ...
    ...mapMutations({
      add: 'increment' // 将 `this.add()` 映射为 `this.$store.commit('increment')`
        ...
        console.log(this.DesktopManage)
        if (this.oldData) {
            this.$nextTick(() => {
                this.form.setFieldsValue({
                    name: this.oldData.name,
                    group: this.oldData.group,
                    software: this.oldData.software,
