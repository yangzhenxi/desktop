  ...
  <div v-if="visible">
    <a-spin :spinning="spinning">
      <a-card :title="modulesData.groupName">
        <a-button slot="extra" @click="handleBack">返回</a-button>
        <a-row :gutter="16">
          <div class="header">
            <a-col :span="14">
              <a-row :gutter="[16,16]">
            ...
            <a-col :span="10">
              <a-row
                ...
                <a-col :span="16">
                  ...
                  <div class="modules_text">
                    <span>创建时间: </span>
                    <span>{{ modulesData.createTime| convert('unix','YYYY-MM-DD') }}</span>
                  </div>
                  <div class="modules_text">
                    <span>版本状态: </span>
                    <a-tag color="#108ee9">
                      {{ modulesData.version | Isversion }}
                    </a-tag>
                  </div>
                  <div
                    class="modules_text"
                    ...
                    <span>运行状态: </span>
                    <!-- :number-style="{backgroundColor: toDict(modulesData.isRunning,'C_STATE').color}" -->
                    <a-badge
                      :count="modulesData.isRunning | convert('C_STATE')" />
                  </div>
                  <div class="modules_text">
                    <span>IP: </span>
      ...
      <a-card>
        <tabs :templateId="templateId" @disabled="Isdisabled" :modulesData="modulesData"></tabs>
      </a-card>
      <x-managerelease
        ref="ManageRelease"
...
export default {
  ...
  methods: {
        ...mapMutations(['SET_MODULESDATA']),
    // 显示详情页面
    async OnSingle (record) {
      this.visible = true
      this.modulesData = this.deepGet(await DesktopGetTemplate({ id: record.id }), 'data', [])
      this.SET_MODULESDATA(this.modulesData)
      this.spinning = false
    },
    // 关闭详情页面
    OffSingle () {
    ...
    Release (res) {
      if (res) {
        this.modulesData.version = parseInt(this.modulesData.version) + 1
      }
