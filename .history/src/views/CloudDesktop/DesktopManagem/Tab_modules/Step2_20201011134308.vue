...
export default {
    ...
    data () {
        ...
        ...mapState({
            ...
            'isCluster': state => state.DesktopManage.isCluster, // 判断是否是集群还是主机
            'Space': state => state.DesktopManage.Space // 内存容量
        })
    },
    mounted () {
        if (Object.keys(this.formValue).length >= 4) {
            this.$nextTick(() => {
                this.form.setFieldsValue({
                    capacityKB: this.formValue.capacityKB / 1024 / 1024,
                    clusterOrHost: this.formValue.clusterOrHost,
                    cpuNum: this.formValue.cpuNum,
                    memoryMB: this.formValue.memoryMB / 1024,
                    network: this.formValue.network,
                    resourcePools: this.formValue.resourcePools,
                    socket: this.formValue.socket,
                    dataCenter: this.formValue.dataCenter
                })
            })
        }
    },
    ...
    methods: {
        ...mapMutations(['SET_FORMVALUE', 'SET_ISCLUSTER']),
        // 解析datacenterList的数据
        getdataCenter (value) {
