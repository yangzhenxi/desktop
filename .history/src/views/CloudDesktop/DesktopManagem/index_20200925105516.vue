<template>
  <page-header-wrapper :title="false">
    <a-spin :spinning="spinning">
      <a-row :gutter="16">
        <a-col :span="6">
          <empty v-if="!TPgroupList.length>0"
          ...
          <a-card v-else>
            <a-row class="tree_head">
              ...
              <a-col :span="14">
                ...
                <a-button :disabled="disabledEdit"
                          type="primary"
                          shape="circle"
            ...
            <div v-else>
              <div v-if="IsAllmodule">
                ...
                <a-card style="height:730px;">
                          ...
                          <a-col class="blurry"
                            ...
                            <div class="progress-position">
                              <a-progress type="circle"
                                ...
                                <template #format="i">
                                  <span class="percent_text">创建中{{ i.progressBar }}%</span>
                                </template>
                              </a-progress>
                            </div>
                          </a-col>
                  ...
                  <!-- 分页器 -->
                  <a-pagination :total="TPData.length+1"
      ...
      <x-manageadd ref="ManageAdd"
                   @ok="ManageAdd()"></x-manageadd>
      <x-managerelease ref="ManageRelease"
  ...
  methods: {
    ...
    async Task() {
      if (this.AllTask.length > 0) {
        ...
        await DesktopTemplateCheckTask(obj).then((res) => {
          const progress = this.deepGet(res, 'progress', [])
          this.progress.push(...progress)
          console.log(this.progress)
        })
        this.progress.forEach((u) => {
          if (u.msg !== '任务全部完成') {
            setInterval(() => {
              const obj = {
                task: this.AllTask,
              }
              DesktopTemplateCheckTask(obj)
            }, 500)
          }
        })
      }
    ...
    closeTask(task) {
      this.$confirm({
        ...
        onOk: async () => {
          return new Promise((resolve, reject) => {
            DesktopTemplateCloseTask().then((res) => {
