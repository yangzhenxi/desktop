            ...
            <a-row class="tree_head">
              ...
              <a-col :span="14" class="flex_end">
                <a-button
                  ...
                  type="primary"
                  shape="circle"
                  @click="$refs.Add.Add(record.key,TPgroupNameList)">
                  <m-icon
                    type="yonghutianjia"
                    class="icon" />
                </a-button>
...
export default {
  ...
  methods: {
      ...
      async getTP () {
          ...
          const [AllTask, GetTP] = (await Promise.all([DesktopTemplateGetAllTask(), DesktopTemplateAllList({ group: parseInt(this.record.id) })].map((u) => u.catch(e => console.log(1)))))
          if (AllTask.length > 0) {
    ...
    TreeDelete () {
      ...
      this.$confirm({
        ...
        content: '点击确定即可删除当前组',
        onOk: async () => {
          await DesktopTemplateGroupDelete({ id: this.TPgroup.key }).then((res) => {
