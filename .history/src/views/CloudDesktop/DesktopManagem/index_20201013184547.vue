...
export default {
  ...
  methods: {
      async getOUGroup () {
          const GroupList = deepGet(await DesktopTemplateGroupList(), 'data', [])
            if (GroupList) {
                ...
                this.record.value = this.record.value
      ...
      async getTP () {
          ...
          const [AllTask, GetTP] = (await Promise.all([DesktopTemplateGetAllTask(), DesktopTemplateAllList({ group: parseInt(this.record.value) })].map((u) => u.catch(e => console.log(1)))))
          if (AllTask.length > 0) {
