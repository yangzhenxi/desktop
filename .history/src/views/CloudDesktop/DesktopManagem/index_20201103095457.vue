...
export default {
    ...
    methods: {
        ...
        async getTP (item) {
            ...
            Task.length > 0 ? this.GetTask(Task) : this.IsDesktopCreating = false
        ...
        async GetTask (arr) {
            ...
            await DesktopTemplateCheckTask(obj).then(res => {
                const result = this.deepGet(res, 'progress', [])
                result.forEach(u => {
                    console.log(this.showTPpageList)
                    console.log(this.AllTPpageList)
                    console.log(this.showTPpageList.findIndex(o => o.name === u.name) !== -1)
                    if (this.showTPpageList.findIndex(o => o.name === u.name) !== -1) {
                        this.showTPpageList.forEach(o => {
                            u.name === o.name ? o.data.cpuIdle = u.progress_bar : console.log(1)
                        })
                    } else if (u.group === this.selectedKeys[0] && u.progress_bar !== 100) {
                        u.data = {
                            input: 10,
                            output: 20,
                ...
                result[0].progress_bar === 100 ? this.getOUGroup(this.record) : console.log(1)
                if (this.Boole) {
                    console.log(1)
                    this.AllTPpageList = [...progress, ...this.TPpageList]
                    this.showTPpageList = this.AllTPpageList.slice(0, 8)
                }
            })
        },
        // 获取系统镜像已经数据中心DataCenterList
        async GetImageList () {
