...
export default {
  ...
  methods: {
    ...
    async Task () {
        console.log()
      if (this.AllTask.task.length > 0) {
        await DesktopTemplateCheckTask(this.AllTask).then((res) => {
        ...
        this.progress.forEach(u => {
            if (u.msg !== '任务全部完成') {
                setInterval(() => {
                    DesktopTemplateCheckTask(this.AllTask).then(res => {
                         const progress = this.deepGet(res, 'progress', [])
                         this.progress.push(...progress)
                    })
                }, 500)
