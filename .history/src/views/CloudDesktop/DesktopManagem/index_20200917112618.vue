<template>
  <page-header-wrapper :title="false">
    <a-spin :spinning="spinning">
      <a-row :gutter="16">
        <a-col :span="6">
          <empty v-if="!TPgroupList.length>0" :isEmpty="!TPgroupList.length>0"/>
          <a-card v-else>
        ...
        <a-col :span="18">
          <!-- <router-view/> -->
          <div>
            <empty v-if="!this.isTPData" :isEmpty="!this.isTPData"/>
            <div v-else>
              <div class="title"> {{ TPname }} </div>
              <a-card style="height:705px;">
                <a-row :gutter="24">
                  <a-col :span="8" @click="$refs.ManageAdd.Add(AddData)">
                  ...
                  <a-col :span="8" v-for=" i in TPData" :key="i.id">
                    <a-row class="modules" :gutter="16">
            ...
            <singlemodules ref="Single" @ok="getTPgroup"></singlemodules>
          </div>
...
export default {
  ...
  methods: {
    ...
    async getTPgroup () {
      this.TPgroupList = []
    // 获取数据中心和系统镜像接口
        const [DataCenterList, ImageList, GroupList, TemplateAllList] = (await Promise.all([DesktopTemplateDataCenterList(), DesktopTemplateImageList(), DesktopTemplateGroupList(), DesktopTemplateAllList()].map(u => u.catch(e => e)))).map(u => deepGet(u, 'data', []))
        if (GroupList) {
