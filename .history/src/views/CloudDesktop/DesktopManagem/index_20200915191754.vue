...
import { DesktopTemplateGroupList, DesktopTemplateDelete, DesktopTemplateAllList } from '@/api/CloudDesktop/DesktopManagem'
export default {
  mixins: [mixinTable],
  components: {
    MIcon,
  ...
  methods: {
      // 获取模版列表信息
      async getTPAllList () {
       const res = await DesktopTemplateAllList()
    ...
    async getTPgroup () {
      ...
      console.log(res.data.children[0].id)
