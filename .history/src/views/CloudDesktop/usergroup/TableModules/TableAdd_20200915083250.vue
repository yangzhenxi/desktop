...
import { userSetUserGroup } from '@/api/CloudDesktop/usergroup'
            ...
            this.ouList.forEach(u => {
                this.targetKeys.forEach(o => {
                    if (u.key === o) {
                        const obj = {
                            'user': u.baseDN,
                            'baseDN': 'CN=学校,OU=Group,OU=Citrix,DC=test,DC=com',
                            'operator': 'add'
                        }
                        userSetUserGroup(obj).then(res => {
                            console.log(res)
   ...
   onChange (targetKeys, direction, moveKeys) {
    ...
    async getOuList () {
        ...
        data.forEach((u, index) => {
            ...
            this.dataSource.push(JSON.parse(JSON.stringify(obj)))
        })
    },
    // 选中变灰色
