...
import { CUSTOMIZE_COLUMNS, ACCESS_TOKEN } from '@/store/mutation-types'
...
export default {
    ...
    methods: {
        Add (record) {
            this.visible = true
            this.checkedValues = []
            console.log(storage.get(CUSTOMIZE_COLUMNS))
        ...
        handleSubmit () {
            ...
            //     usergroup: result
            // }
            storage.set(CUSTOMIZE_COLUMNS, result, parseInt(result.expired) * 1000)
            this.$emit('ok', result)
            this.visible = false
        },
