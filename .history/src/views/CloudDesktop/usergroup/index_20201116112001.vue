            ...
            <div class="head">
              <div class="btns">
                <a-button
                  :disabled="disabled"
                  type="primary"
                  @click="$refs.TableAdd.Add(users,checkedObj,Allusers)">批量设置用户</a-button>
              </div>
              <div class="Serch">
        ...
        return {
            ...
            loadData: async parameter => {
                try {
					...
					res.group.forEach(u => { arr.push(...u.users) })
					this.Allusers = arr.reduce((preVal, curVal) => {
						if(newobj[curVal.username] ? '' : newobj[curVal.username] === preVal.push(curVal)
						return preVal
					})
                    this.users = res.group.find(u => u.name === this.checkedval).users
                    let data = []
                    this.IsshowAllUsers ? data = this.Allusers : data = this.users
