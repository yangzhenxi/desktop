<template>
  <page-header-wrapper :title="false">
    ...
    <tree-add ref="TreeAdd" @ok="getListUserGroup()"></tree-add>
    <tree-edit ref="TreeEdit" @ok="getListUserGroup()"></tree-edit>
...
import { usermanageListusergroup, userDelUserGroup } from '@/api/CloudDesktop/usergroup'
const columns = [
  {
    title: '用户名',
    dataIndex: 'username',
...
export default {
    ...
    return {
      ...
      loadData: async (parameter) => {
        try {
            ...
            if (this.checkedval.users === undefined) {
                const data = this.deepGet(await usermanageListusergroup().group[0], 'users', [])
                // const data = await usermanageListusergroup()
                console.log(data)
                return {
                    data,
  ...
  methods: {
    ...
    DeleteTree () {
        if (this.checkedval !== '') {
            this.$confirm({
                title: '是否删除' + this.checkedval.name,
                content: '点击确定即可删除组',
                onOk: () => {
                    const obj = {
                        baseDN: this.checkedval.baseDN
                    }
    ...
    handleTreeEdit () {
        if (this.checkedval !== '') {
            this.$refs.TreeEdit.Edit(this.checkedval)
        } else {
            this.$message.info('请先选择对应的用户组')
        }
    }
