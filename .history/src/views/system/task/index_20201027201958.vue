  ...
  <page-header-wrapper :title="false">
    ...
    >
      ...
      <a-timeline>
        ...
        <a-timeline-item v-if="Drawerdata.steps">
          <p> {{ Drawerdata.title }}</p>
          <p style="display:flex;justify-content: space-between">
            <span>{{ Drawerdata.state | convert('C_D_DESKTOP_TASK_ATSTE') }}</span>
            <a-progress v-if="u.state!== 'SYSTEM_TASK_STATE_SUCCESS'" :percent="parseInt(Drawerdata.progress)" status="active" style="width:80%;" />
          </p>
        </a-timeline-item>
      </a-timeline>
    </a-drawer>
